...
//IMU串口DMA接收缓数组存大小
#define Recieve_IMU_Port_SIZE 23
...
extern xQueueHandle Recieve_IMU_Port;
//串口缓冲区
extern uint8_t Uart3_Rx_Buff[Recieve_IMU_Port_SIZE];

/////////////////////////////////////////底盘控制/////////////////////////////////////////
	
//机器人底盘运动模式枚举
typedef enum CHASSIS_MODE//偷py屁眼
{
	X_MOVE,
	Y_MOVE,
	NORMAL
...
//底盘状态枚举
typedef enum CHASSIS_STATUS//py屁眼很大
{
	OFF,
	INIT,
	ON
...
//机器人底盘运动参数结构体
typedef struct Robot_Twist_t//py屁眼很小
{
	struct {
		float x;
		float y;
...
//状态枚举
typedef enum PLAYLIST// py真的有屁眼嘛？
{
	//比赛时间点
	STOP,
	ROBOT_START=1,
...
//状态结构
typedef struct Robot_Status_t//卧槽py屁眼真的大
{
	PLAYLIST robot_init;//机器人初始化
	PLAYLIST path_mode;//路径模式
	PLAYLIST sensor;//传感器模式
	PLAYLIST control_mode;//控制模式
}Robot_Status_t;
...
//CAN发送数据结构体
typedef struct CAN_TxMsg// py屁眼很小吧？
{    
	  uint32_t id;	//ID
    uint8_t len;	//数据长度
