...
/**
 * @brief 定时器获取函数初始化，在config文件中进行调用
 * 
 * @param getTick_fun 
 * @return uint8_t 
 */
...
void Swerve_Chassis::Control(Robot_Twist_t cmd_vel)
{
    ...
    for(int i=0; i<4; i++)
    {
        if(chassis_is_init==true&&Chassis_Safety_Check(25000)==true)
        {
            ...
            //使用加速度控制底盘速度,
            #if USE_VEL_ACCEL
            if(swerve[i].wheel_vel > 0 && swerve[i].wheel_vel >= last_wheel_vel[i])//如果当前轮子的速度大于0且当前速度大于或等于上一次的速度，则说明轮子正在加速。
                swerve[i].wheel_vel = last_wheel_vel[i] + accel_vel*dt*ChassisVel_Trans_MotorRPM(Wheel_Radius, 21);
            else if (swerve[i].wheel_vel < 0 && swerve[i].wheel_vel <= last_wheel_vel[i])//如果当前轮子的速度小于0且当前速度小于或等于上一次的速度，则说明轮子正在减速。
                swerve[i].wheel_vel = last_wheel_vel[i] - accel_vel*dt*ChassisVel_Trans_MotorRPM(Wheel_Radius, 21);
            else
            {}
...
/**
 * @brief 把底盘锁死X方向 但是能否走的直，取决于舵向精度
 * 
 * @param cmd_vel 
 * @param swerve 
 */
...
/**
 * @brief 把底盘锁死Y方向 但是能否走的直，取决于舵向精度
 * 
 * @param cmd_vel 
 * @param swerve 
 */
