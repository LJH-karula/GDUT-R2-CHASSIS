...
int8_t ROS::Recieve_From_ROS_Pos(uint8_t *buffer)
{
    ...
    PID_position.target = length - distance; // 设置PID控制器的目标值为当前位置距离目标距离的差值，target.current已经初始化为0
...
void ROS::Pid_Param_Init(float kp, float ki, float kd,float Integral_Max, float Out_Max, float DeadZone)
{
    PID_position.PID_Param_Init(kp, ki, kd, Integral_Max, Out_Max, DeadZone);
}

void ROS::Pid_Mode_Init(float LowPass_error, float LowPass_d_err, bool D_of_Current, bool Imcreatement_of_Out)
{
