...
extern Motor_GM6020 RudderMotor[4];  //转向
extern VESC WheelMotor[4];           //舵轮
...
public:
    ...
    {
        this->Wheel_Radius = Wheel_Radius; //轮子半径
        this->Wheel_Track = Wheel_Track;   //轮子间距
        this->Chassis_Radius = Chassis_Radius;//底盘半径
        this->wheel_num = wheel_num;//轮子数量
        this->cmd_vel_.chassis_mode = NORMAL;//底盘模式
        swerve[0].num = 1;
        swerve[1].num = 2;
        swerve[2].num = 3;
        swerve[3].num = 4;
...
private:
    int wheel_num = 0;
    Swerve_t swerve[4];
    float Wheel_Radius = 0.038;//轮子半径
    float Wheel_Track = 0;//轮子间距
    float Chassis_Radius = 0.641/2;//底盘半径
